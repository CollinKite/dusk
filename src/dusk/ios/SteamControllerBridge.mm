// Steam Controller -> SDL virtual gamepad bridge (iOS).
//
// The Steam Controller streams its inputs over a custom Bluetooth LE service
// that SDL does not recognize, so it is never seen as a gamepad on iOS. This
// bridge connects to that service directly, decodes the controller's input
// reports, and feeds them into an SDL virtual joystick described as a standard
// gamepad. From there the game's normal SDL gamepad path handles it.
